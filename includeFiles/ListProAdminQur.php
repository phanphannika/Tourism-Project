<?php
  include ('dbCon.php');
  $qwAll = 'SELECT * FROM tbldata td INNER JOIN tblcategory tc ON td.CateID=tc.CateID ORDER BY DateIn ASC';
  ...
  // output data of each row
  {
    ...
    {  
      echo '<tr style="border-bottom: medium #04D5EF solid; border-top: medium #04D5EF solid">';
        ...
        echo '<td style="vertical-align: top; padding:5px">'. $row["CateID"] .'</td> ';
        echo '<td style="vertical-align: top; padding:5px">'. $row["Description"] .'</td> ';        
        
        echo '<td style="vertical-align: top; padding:5px; text-align: center"> <img src="'. $row["ImageUrl"] .'" width="150px" /></td> ';
